...
class SynonymTransform(RuleTransform):
    ...
    def transform(self, sentence): 
        ...
        for _ in range(self.aug_num):
            ...
            for idx in indices:
                ...
                candidate_sent = self.substitute(candidate_sent.strip(), idx)
    ...
    def wordnet_substitute(self, sentence, idx): 
        # print(self.hownet_dict)
        # print(idx)   
        try:   
        ...
        for synonym in synonyms:
            ...
            # deduplicate
            if candidate_sent.lower() == sentence.lower() or candidate_sent in self.sents:
                continue
        ...
        if len(candidate_sents) >= 1:
            return random.choice(candidate_sents)
        else:
            return None

    def hownet_substitute(self, sentence, idx):
    ...
    def get_hownet_dict(self,sentence):
        ...
        for i, word in enumerate(words):
            ...
            temp = deepcopy(filtered_replacements)
            for t in temp:
                if len(t.split())>1:
                    filtered_replacements.remove(t)
